...
public class MorrisSampler
{
    ...
    private double[] regionCodes = null;
    ...
    public void setup()
    {
        originalEcocomps=makeLayerListClone(MappingProject.ecocomps);
        originalStressors=makeLayerListClone(MappingProject.stressors);
        originalScores=MappingProject.sensitivityScores.clone(originalStressors, originalEcocomps);
        originalRegions=MappingProject.regions.clone();
    }
    ...
    public MappingResults calculateOutputs(double[] parameters)	
    {
        ...
        //MappingProject.results.add(simulator.getResult());
        ArrayList<double[]> regionResults = simulator.getRegionCodesAndRanks(originalRegions.getGrid().getData()); //return region ranks in order in which region codes appear
    ...
    /*public MappingResults calculateOutputs(double[] parameters, double[] lastParameters)	
    {
        ...
        scores = MappingProject.sensitivityScores.clone(stressors, ecocomps);
        
        //ecocomps need only be changed if analysis resolution changes
        if(parameters[4]!=lastParameters[4])
        {
            ecocomps = makeLayerListClone(MappingProject.ecocomps);
        }
        ...
        if(parameters[4]!=lastParameters[4] || parameters[7]!=lastParameters[7])
        {
            ...
            stressors = makeLayerListClone(MappingProject.stressors);
            MCSimulationManager.eraseProcessingChains(stressors);
       
            //make changes according to model factors
            int activeStressors = MappingProject.stressors.size();
        ...
        else if(parameters[2]!=lastParameters[2])
        {
            ...
            stressors = MCSimulationManager.makeLayerListCloneBySpatialDataType(MappingProject.stressors,GlobalResources.SPATIALDATATYPE_POINT);
        ...
        else if(parameters[5]!=lastParameters[5])
        {
            ...
            stressors = MCSimulationManager.makeLayerListCloneBySpatialDataType(MappingProject.stressors,GlobalResources.SPATIALDATATYPE_POLYGON);
        ...
        ArrayList<double[]> regionResults = simulator.getRegionCodesAndRanks(MappingProject.regions.getGrid().getData()); //return region ranks in order in which region codes appear
        regionCodes = regionResults.get(0);
        results.regionRanks=regionResults.get(1);
        results.stressorRanks=simulator.getStressorRanks();
        results.ecocompRanks=simulator.getEcocompRanks();
    ...
    protected void changeSensitivityScores(SensitivityScoreSet scores, double parameter)
    {
        ...
        {
            scores.getAllScores().get(i).changeSensitivtyScore(scores.getInfo(i).getSensitivityScore()+parameter*this.scoreErrors[i]);
    ...
    public ElementaryEffects calculateElementaryEffects(double[][] orientationMatrix)
    {
       ...
       //[steps][regions]
        double[][] regionOutputs = new double[orientationMatrix.length][originalRegions.grid.getUniqueDataValues().size()]; 
        double[][] stressorOutputs = new double[orientationMatrix.length][originalStressors.size()]; 
        double[][] ecocompOutputs = new double[orientationMatrix.length][originalEcocomps.size()]; 
        ...
        for(int i=0; i<orientationMatrix.length-1;i++)
        {
            ...
            double[] regionOutputs1 = regionOutputs[i];
            double[] regionOutputs2 = regionOutputs[i+1];
            double[] stressorOutputs1 = stressorOutputs[i];
            double[] stressorOutputs2 = stressorOutputs[i+1];
            double[] ecocompOutputs1 = ecocompOutputs[i];
            double[] ecocompOutputs2 = ecocompOutputs[i+1];
    ...
    {
        /*//[trajectory][parameter][region]
        regionEEMatrices = new double[sampleSize][this.parameterValues.length][MappingProject.regions.getGrid().getUniqueDataValues().size()];
        stressorEEMatrices = new double[sampleSize][this.parameterValues.length][MappingProject.stressors.size()];
        ecocompEEMatrices = new double[sampleSize][this.parameterValues.length][MappingProject.ecocomps.size()];
        for(int r=0; r<sampleSize; r++)
        {
