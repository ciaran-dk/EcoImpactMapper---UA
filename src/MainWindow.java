...
public class MainWindow extends javax.swing.JFrame {
        ...
        if (result == JFileChooser.APPROVE_OPTION)
        {
            ...
            SwingWorker<ImpactIndexAdditive, Void> worker = new SwingWorker<ImpactIndexAdditive, Void>() 
            {
                 ...
                 protected ImpactIndexAdditive doInBackground() throws Exception 
                 {
                    ...
                    ImpactIndexAdditive index = new ImpactIndexAdditive(selectedFile.getAbsolutePath(),MappingProject.sensitivityScores, true);
    ...
    private void menuImpactIndexActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuImpactIndexActionPerformed
        ...
        if (result == JFileChooser.APPROVE_OPTION)
        {
            ...
            SwingWorker<ImpactIndexAdditive, Void> worker = new SwingWorker<ImpactIndexAdditive, Void>() 
            {
                 ...
                 {
                    //System.out.println("**** Started background thread...");
                    ImpactIndexAdditive index = new ImpactIndexAdditive(selectedFile.getAbsolutePath(),MappingProject.sensitivityScores, false);
                    MappingProject.processing=false;
                    return index;
                }
    ...
    private void menuItemExportLayerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuItemExportLayerActionPerformed
        ...
        if (result == JFileChooser.APPROVE_OPTION)
        {
            ...
            SpatialDataLayer selectedLayer=MappingProject.getDataLayerByName(selectedLayerName);
            ...
            else
            {
                ...
                {
                    table=MappingProject.grid.createTableFromLayer(selectedLayer, true);
                }
                else
                {
                    table=MappingProject.grid.createTableFromLayer(selectedLayer, false);
                }
                table.writeToFile(selectedFile.getAbsolutePath());
            }
    ...
    private void menuItemExportPolyAreasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuItemExportPolyAreasActionPerformed
        ...
        if (result == JFileChooser.APPROVE_OPTION)
        {
            ...
            else
            {
                ...
                double[][] areaCodesDouble=new double[areaCodes.length][areaCodes[0].length];
                for(int x=0; x<areaCodes.length;x++)
                    for(int y=0;y<areaCodes[0].length;y++)
                    {
                        areaCodesDouble[x][y]=areaCodes[x][y];
                    }
                
                DataGrid areaGrid = new DataGrid(areaCodesDouble,selectedLayer.getGrid().getPresenceAreaSizes().length,0,selectedLayer.getGrid().getNoDataValue());
                SpatialDataLayer areaLayer=new SpatialDataLayer("areas",areaGrid,0,new DataSourceInfo());
                
                CsvTable table=MappingProject.grid.createTableFromLayer(areaLayer, false);
                table.writeToFile(selectedFile.getAbsolutePath());
            } 
        }
    ...
    private void menuItemMonteCarloRanksActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuItemMonteCarloRanksActionPerformed
        ...
        if(mcDialog.isSimulationReady())
        {
           ...
           if(!multiThreading)
           {
                ...
                System.out.println("Calculated "+mcm.simulationRuns+" elementary effects in "+duration*0.001*(1.0/60)*(1.0/60)+" hours.");
           }

           else
           {
               MCSimulationWorker mc1=new MCSimulationWorker();
               MCSimulationWorker mc2=new MCSimulationWorker();
               mc1.sampleSize=(int) Math.ceil(0.5*mcm.simulationRuns);
               mc2.sampleSize=(int) Math.ceil(0.5*mcm.simulationRuns);
               mc1.workerNr=1;
               mc2.workerNr=2;
               mc1.setSimulationManager(mcm);
               mc2.setSimulationManager(mcm.clone());
               mc1.working=true;
               mc2.working=true;
              ...
              try
              {
                    ...
                    //Thread.sleep(500); //
                    MCSimulationManager mcm1 = mc1.get();
                    MCSimulationManager mcm2 = mc2.get();
                    Thread.sleep(1000);
                    System.out.println("Merging thread results...");
                    mcm1.mergeResults(mcm2);
              ...
              {
                  JOptionPane.showMessageDialog(this, "Multithreading error.");
              }
           }
        }
    ...
    private void menuItemImpactIndexDominantSumActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuItemImpactIndexDominantSumActionPerformed
        ...
        if (result == JFileChooser.APPROVE_OPTION)
        {
            ...
            SwingWorker<ImpactIndexDominant, Void> worker = new SwingWorker<ImpactIndexDominant, Void>() 
            {
                 ...
                 {
                    //System.out.println("**** Started background thread...");
                    ImpactIndexDominant index = new ImpactIndexDominant(selectedFile.getAbsolutePath(),MappingProject.sensitivityScores, false);
                    MappingProject.processing=false;
                    return index;
                }
    ...
    private void jMenuItem2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItem2ActionPerformed
        ...
        if (result == JFileChooser.APPROVE_OPTION)
        {
            ...
            SwingWorker<ImpactIndexDominant, Void> worker = new SwingWorker<ImpactIndexDominant, Void>() 
            {
                 ...
                 {
                    //System.out.println("**** Started background thread...");
                    ImpactIndexDominant index = new ImpactIndexDominant(selectedFile.getAbsolutePath(),MappingProject.sensitivityScores, true);
                    MappingProject.processing=false;
                    return index;
                }
    ...
    private void menuItemDiminishingImpactsSumActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuItemDiminishingImpactsSumActionPerformed
        ...
        if (result == JFileChooser.APPROVE_OPTION)
        {
            ...
            SwingWorker<ImpactIndexDiminishing, Void> worker = new SwingWorker<ImpactIndexDiminishing, Void>() 
            {
                 ...
                 {
                    //System.out.println("**** Started background thread...");
                    ImpactIndexDiminishing index = new ImpactIndexDiminishing(selectedFile.getAbsolutePath(),MappingProject.sensitivityScores, false);
                    MappingProject.processing=false;
                    return index;
                }
    ...
    private void menuItemDiminishingImpactMeanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuItemDiminishingImpactMeanActionPerformed
        ...
        if (result == JFileChooser.APPROVE_OPTION)
        {
            ...
            SwingWorker<ImpactIndexDiminishing, Void> worker = new SwingWorker<ImpactIndexDiminishing, Void>() 
            {
                 ...
                 {
                    //System.out.println("**** Started background thread...");
                    ImpactIndexDiminishing index = new ImpactIndexDiminishing(selectedFile.getAbsolutePath(),MappingProject.sensitivityScores, true);
                    MappingProject.processing=false;
                    return index;
                }
    ...
    private void jMenuItem4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItem4ActionPerformed
        ...
        {
            File selectedFile = fileChooser.getSelectedFile();
            MappingProject.saveLowerResolutionVersion(selectedFile.getAbsolutePath(),reductionFactor);
        }
    ...
    private void menuItemRunCurrentTestActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuItemRunCurrentTestActionPerformed
       //got through all stressors; make a clone; calculate unique values; and compare results using 2 methods
        ...
        {
            DataGrid grid1 = MappingProject.stressors.get(i).getGrid().clone();
            DataGrid grid2 = MappingProject.stressors.get(i).getGrid().clone();
