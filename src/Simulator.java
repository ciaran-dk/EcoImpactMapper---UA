...
class Simulator 
{
    ...
    //The other version below uses internal variables, which is preferable for multi-threading.
    public Simulator(ArrayList<SpatialDataLayer> stressors, ArrayList<SpatialDataLayer> ecocomps, SpatialDataLayer regions, SensitivityScoreSet scores, int mem, int meanOrSum, ArrayList<StressorRankInfo> stressorInfos, ArrayList<EcocompRankInfo> ecocompInfos, ArrayList<RegionRankInfo> regionInfos)
    {
    ...
    //calculate impact index
     //now create internal data grid
        ...
        {
            ecocompSum = new double[MappingProject.grid.getDimensions().x][MappingProject.grid.getDimensions().y];
        }
        else
        {
            ...
            if(impact.isActive())
            {
                ...
                for(int x=0;x<ecocompData.length;x++)
                {
                    for(int y=0; y<ecocompData[0].length; y++)
                    {
                        ...
                        {
                            double summand=score*ecocompData[x][y]*r.getResponse(stressorData[x][y]);
    ...
    private void calculateWithDominantMEM() 
    {
        ...
        double[][] values = MappingProject.grid.getEmptyGrid();
        ...
        for(int e=0; e<ecocomps.size();e++)
        {
            ...
            double[][] ecocomp = ecocomps.get(e).getProcessedGrid().getData();
            ...
            for(int x=0; x<MappingProject.grid.getDimensions().x;x++)
            {
                for(int y=0; y<MappingProject.grid.getDimensions().y;y++)
                {
                    if(values[x][y]!=GlobalResources.NODATAVALUE && ecocomp[x][y]>0)  //only for present ecocomps
                    {
                        double largestSummand = -1;
                        int largestIndex=-1;
    ...
    private void calculateWithDiminishingMEM() 
    {
        ...
        for(int e=0; e<ecocomps.size();e++)
        {
            MappingProject.processingProgressPercent = (int) (100*e/ecocomps.size());
            //get impacts only for this ecocomp
            ArrayList<ImpactInfo> eImpacts = scores.getActiveImpactsForEcocomp(ecocomps.get(e).getName());
            double[][] ecocomp = ecocomps.get(e).getProcessedGrid().getData();
            //now go through the grid cell by cell... for each cell, find most important stressor, that's the summand, divide by # of ecocomps if need be, add to result
    ...
    public ArrayList<double[]> getRegionCodesAndRanks(double[][] regionCodes)
    {
        ArrayList<RegionRankInfo> regions = new ArrayList<RegionRankInfo>();
        double[][] impacts = getResult().getGrid().getData();
        ...
        double[] codes = new double[regions.size()];
        double[] regionRanks = new double[regions.size()];
        for(int i=0; i<regions.size();i++)
        {
            RegionRankInfo info = regions.get(i);
  ...
  /*  //can be external references to static variables to keep infos across simulator instances
    ...
    private void initializeInfoObjects() 
    {
        ...
        ArrayList<Double> regionCodes = regions.grid.getUniqueDataValues();
   
        regionInfos = new ArrayList<RegionRankInfo>();
        double[][] regionData = regions.grid.getData();
    ...
    //returns stressor ranks in order that stressors appear in MappingProject, with
    //no data value
    public double[] getStressorRanks()
    {
        
        double[] ranks = new double[stressors.size()];
        ...
        if(stressorInfos!=null)
        {
            ...
            {
                if(ranks[i]!=GlobalResources.NODATAVALUE) {ranks[i]=(1.0*ranks[i])/active;}
            }
        }
        return ranks;
    }
    
        //returns ecocomp ranks in order that stressors appear in MappingProject
    public double[] getEcocompRanks()
    {
        
        double[] ranks = new double[ecocomps.size()];
