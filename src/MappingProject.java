...
public abstract class MappingProject 
{
    ...
    public static SpatialDataLayer regions = null;
    ...
    static void saveLowerResolutionVersion(String filename, int reductionFactor) 
    {
        ...
        CsvTable masterTable = grid.createTableFromLayer(stressors2.get(0), true, grid.getCellSize()*reductionFactor);
        for(int i=1; i<stressors2.size();i++)
        {
            System.out.println("Reducing resolution for stressor " + (i+1) + " of "+stressors2.size());
            CsvTable newTable = grid.createTableFromLayer(stressors2.get(i), true, grid.getCellSize()*reductionFactor);
            masterTable.append(newTable);
        }
        for(int i=0; i<ecocomps2.size();i++)
        {
            System.out.println("Reducing resolution for ecological component " + (i+1) + " of "+ecocomps2.size());
            CsvTable newTable = grid.createTableFromLayer(ecocomps2.get(i), true, grid.getCellSize()*reductionFactor);
            masterTable.append(newTable);
        }
    ...
    private static void saveDataTypes(String filename)
    {
        CsvTable table = new CsvTable();
        table.addColumn("DataLayer");
    ...
    public static void createFromTable(CsvTable table)
    {
        ...
        final CsvTable dataTypeTable=new CsvTable();
    ...
    public static DrawableData getResultByName(String resultName) 
    {
